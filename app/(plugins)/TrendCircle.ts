...
class CirclePaneRenderer implements IPrimitivePaneRenderer {
	...
	draw(target: CanvasRenderingTarget2D): void {
		target.useBitmapCoordinateSpace((scope: BitmapCoordinatesRenderingScope) => {
			...
			ctx.beginPath();
			ctx.arc(x * scope.horizontalPixelRatio, y * scope.verticalPixelRatio, radiusPx, this._options.startAngle ?? 0, this._options.endAngle ?? 2 * Math.PI , true);
...
class CirclePaneView implements IPrimitivePaneView {
	...
	renderer(): IPrimitivePaneRenderer {
		return new CirclePaneRenderer(this._center, this._source._radius, this._source._options);
	}
}
...
export class CirclePrimitive implements ISeriesPrimitive<Time> {
	...
	constructor(
		...
		this._options = {
			...options,
			startAngle: options.startAngle !== undefined ? degToRad(options.startAngle) : 0,
			endAngle: options.endAngle !== undefined ? degToRad(options.endAngle) : 2 * Math.PI,
		};
