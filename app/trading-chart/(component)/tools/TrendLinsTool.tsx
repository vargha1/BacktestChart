...
function TrendLinsTool() {
  ...
  const [tool, setTool] = useState<ITool | null>({
    title: Tools[0].children[0].title,
    icon: Tools[0].children[0].icon,
    _id: Tools[0].children[0]._id,
  });
  ...
  const handleClick = (param: ITool | null) => {
    if (!param) return;
    ...
    <div ref={menuRef}>
      <div className="flex items-center justify-between gap-1">
        ...
        <div className="rounded-md hover:bg-[#2E2E2E] px-1">
          <ButtonComponent
            text={<FaChevronRight className="w-full h-full" />}
            className={"text-[8px] h-full"}
            onclick={() =>
          ...
          <div className="bg-[#1F1F1F] absolute top-0 left-12  z-50 rounded-md h-[90vh] md:w-[25vw] w-[40vw] overflow-scroll scrollbar scrollbar-thumb-rounded-md scrollbar-w-1">
            <ul>
              {Tools.map((tool, index: number) => (
                <li key={index}>
                  <span className="text-[#767677] text-[12px]">
                    {tool.type}
                  </span>
                  <ul className="">
                    {tool.children.map((item, index: number) => (
                      <li
                        key={index}
                        className="flex hover:bg-[#2E2E2E] justify-between px-5"
                      >
                        ...
                        <ButtonComponent
                          text={<IoIosStarOutline />}
                          onclick={setFavoriteTool}
                        />
                      </li>
                    ))}
                  </ul>
