...
const timeFrames = [
    { title: 'second', intervals: ['1 second', '5 seconds', '10 seconds', '15 seconds', '30 seconds', '45 seconds'] },
    { title: 'minutes', intervals: ['1 minute', '2 minutes', '3 minutes', '5 minutes', '10 minutes', '15 minutes', '30 minutes', '45 minutes'] },
    { title: 'hours', intervals: ['1 hour', '2 hours', '3 hours', '4 hours', '6 h', '8 h', '12 h'] },
...
function TimeFrames() {
    ...
    return (
        <div className='relative' ref={menuRef}>
            <ButtonComponent text={'1m'} className='text-white hover:bg-[#3D3D3D] rounded-md' onclick={handelShow} />
            {showTimeFrames && (
                <div className='z-50 absolute top-0 bg-[#211F21] w-[200px] md:max-h-[100vh] overflow-y-scroll scrollbar scrollbar-thumb-[#565353] scrollbar-w-1 scrollbar-thumb-rounded-md p-3 rounded-md'>
                    <div>
                        <ButtonComponent text={'➕ add custom intervals'} className={'md:text-[12px]'} onclick={openModal} />
                    </div>
                    {timeFrames.map((time, index) => (
                        <div key={index}>
                            <span className='text-[#767677] text-[13px]'>{time.title.toUpperCase()}</span>
                            <ul>
                                {time.intervals.map((item, index) => (
                                    <li key={index}>
                                        <ButtonComponent text={item} className={'hover:bg-[#3D3D3D] text-[#efefef] w-full text-start text-[14px]'} />
                                    </li>
                                ))}
                            </ul>
                ...
                >
                    <form className='flex flex-col gap-5 p-5'>
                        ...
                        <div className='flex justify-between w-3/4'>
                            ...
                            <select name="typeTime" id="type" className='border px-5 rounded-md'>
                        ...
                        <div className='flex justify-between w-3/4 gap-3'>
                            ...
                            <input type="number" name="interval" id="interval" className='border rounded-md px-3 w-[130px]' />
                        </div>
                        <hr />
                        <div className='flex justify-around'>
                            <ButtonComponent text={'add'} visible={false} type={'submit'}/>
                            <ButtonComponent text='cancel' type={'button'} onclick={closeModal}/>
                        </div>
                    </form>
                </ModalComponent>
            )}
